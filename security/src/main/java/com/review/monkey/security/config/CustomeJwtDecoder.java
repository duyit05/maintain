...
public class CustomeJwtDecoder implements JwtDecoder {
    ...
    public Jwt decode(String token) throws JwtException {
        try {
            IntrospectResponse isValidToken = authenticationService.introspect(
                    IntrospectRequest.builder().token(token).build());

            if (!isValidToken.isValid()) throw new AppException(ErrorCode.UNAUTHENTICATED);
        ...
        if (Objects.isNull(nuNimbusJwtDecoder)) {
            ...
            nuNimbusJwtDecoder = NimbusJwtDecoder.withSecretKey(secretKeySpec)
