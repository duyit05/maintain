...
public class PermissionImplement implements PermissionSerivce {
    ...
    public PermissionResponse updatePermissionById(String permissionId, PermissionRequest request) {
        Permission checkExisted = permissionRepository
                .findById(permissionId)
                .orElseThrow(() -> new RuntimeException("CAN'T UPDATE PERMISSION WITH ID : " + permissionId));
    ...
    public void deletePermissionById(String permissionId) {
        Permission checkExisted = permissionRepository
                .findById(permissionId)
                .orElseThrow(() -> new RuntimeException("CAN'T DELETE PERMISSION WITH ID : " + permissionId));
        permissionRepository.delete(checkExisted);
    }

    @Override
    public PermissionResponse getPermissionById(String permissionId) {
        return permissionMapper.toPemissionResponse(permissionRepository
                .findById(permissionId)
                .orElseThrow(() -> new RuntimeException("CAN'T DELETE PERMISSION WITH ID : " + permissionId)));
    }
}
