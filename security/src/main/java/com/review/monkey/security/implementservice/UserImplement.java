...
public class UserImplement implements UserService {
    ...
    public UserResponse createUser(UserRequest request) {
        ...
        Role roleDefault = roleRepository
                .findByRoleName("USER")
                .orElseThrow(() -> new AppException(ErrorCode.ROLE_NAME_NOT_FOUND));

        UserRole userRole = UserRole.builder().user(user).role(roleDefault).build();
    ...
    public void deleteUser(String userId) {
        User user = userRepository
                .findById(userId)
                .orElseThrow(() -> new RuntimeException("CAN'T DELETE USER WITH ID : " + userId));
    ...
    public Page<UserResponse> getUserPagingAndSearch(UserSearchRequest request) {
        Page<User> users = userRepository.findAll(
                request.specification(), request.getPaging().pageable());
